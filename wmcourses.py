...
def main():
	...
	# the url for the list of courses
	...
	for link in soup.find_all('td'):
		...
		if not text.isspace():
			...
			# if we are processing a courses' time info, convert time to standard time
			if key is 'TIMES':
				time = text.split('-', 2)
		...
		if field_count == 12:
			...
			# reset the current field count and entry and increment the course count
			field_count = 0
			course_count += 1
			entry = {}
